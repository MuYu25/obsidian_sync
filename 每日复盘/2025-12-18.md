...
```
- 补充一点缓存策略，`redis`不能保证不挂的，所以策略在每日更新中`redis`挂了或者缓存不存在，会去查数据库然后回填到缓存中。而查询只判断缓存不存在，存在直接用缓存，缓存不存在则去数据库查数据。
## **今天有什么事情可以做得更好？卡点在哪里？**（目的：发现问题，但不是自责。例如：“沟通时没先确认对方需求，导致返工”）
- 今天有被坑的点，改动是我在后期接手的。导致最后更新上去时，没有找上游同事拿`DDL`语句，然后我还以为是统一更新一些表对多应用的支持。然后就导致今天耽搁一天没有去让正式更新数据.早上刚来就返反馈了问题，快下班才解决。
- 接手同事的改动也是一个大坑，完全不知道还要注意什么。被坑死了，要是谁有主要责任，那么这个人就会是我。我勒个雷啊。
## **基于以上，我明天可以立即尝试的一个小改进是什么？**（目的：将复盘转化为行动。例如：“明天开会前，先花2分钟写下核心目标和问题”）
- `count`确实大多数就是不太可能报错的，除非语句出现了问题，才可能报错。下次留意。可以省自己的时间。
- 不能或者少接同事的改动，完全不知道还有什么，还有什么要改的，这一次就是没问同事要`DDL`语句，被坑了，还容易背锅。